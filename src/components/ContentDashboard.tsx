...
const ContentDashboard = () => {
  ...
  const grantTimeCounts = dataCapSo.reduce((counts: any, item) => {
    let date;
    if (chartPeriod === "day") {
      date = moment(item.grantTime, "HH:mm DD/MM/YYYY").format("DD");
    } else if (chartPeriod === "week") {
      date = moment(item.grantTime, "HH:mm DD/MM/YYYY").format("WW");
    } else if (chartPeriod === "month") {
      date = moment(item.grantTime, "HH:mm DD/MM/YYYY").format("MM");
    }
    if (date !== undefined) {
  ...
  const getChartLabels = () => {
    if (chartPeriod === "day") {
      return Object.keys(grantTimeCounts);
    } else if (chartPeriod === "week") {
      const weekNumbers = Object.keys(grantTimeCounts).map(Number);
      return weekNumbers;
    } else if (chartPeriod === "month") {
      const monthNumbers = Object.keys(grantTimeCounts).map(Number);
      return monthNumbers;
    }
  };
  ...
  return (
    <div className="">
      ...
      <Select defaultValue={chartPeriod} onChange={handleChartPeriodChange}>
