...
import { Badge, Calendar, Card, Col, Layout, Row } from "antd";
...
const Dashboard: React.FC = () => {
  ...
  const dataSevice = useSelector((state: RootState) => state.service.services);
  ...
  useEffect(() => {
    const totalService = dataSevice.length;
    const activeService = dataSevice.filter(
      (item) => item.isActive === true
    ).length;

    const isActiveService = dataSevice.filter(
                ...
                <Link to={`/devices`}>
                  <Row className="d-flex justify-content-between">
                    <Col>
                      ...
                      >
                        <CircularProgressbar
                          value={totalPercentDevice}
                          minValue={0}
                          maxValue={100}
                          text={`${totalPercentDevice.toFixed(0)}%`}
                        />
                      </div>
                    </Col>
                    <Col>
                ...
                <Link to={`/services`}>
                  <Row className="d-flex justify-content-between">
                    <Col>
                      ...
                      >
                        <CircularProgressbar
                          value={totalPercentService}
                          minValue={0}
                          maxValue={100}
                          text={`${totalPercentService.toFixed(0)}%`}
                        />
                      </div>
                    </Col>
                    <Col>
                    ...
                    <Col>
                      ...
                      >
                        <CircularProgressbar
                          minValue={0}
                          maxValue={100}
                          value={totalPercentCapSo}
                          text={`${totalPercentCapSo.toFixed(0)}%`}
                        />
                      </div>
                    </Col>
                    <Col>
